...
class Project(object):
    def __init__(self,
        ...
        self._path = None
        self._tenanetId = tenanetId
        self._botId = botId
        self.remote_storage = remote_storage

        if tenanetId and botId and project_dir:
            self._path = os.path.join(project_dir, tenanetId)
    ...
    def parse(self, text, time=None, requested_model_name=None):
        ...
        response['tenanetId'] = self._tenanetId
        response['botId'] = self._botId
    ...
    def _list_models_in_cloud(self):
        ...
        try:
            ...
            if p is not None:
                return p.list_models(self._tenanetId ,self._botId)
            else:
                return []
        except Exception as e:
            logger.warn("Failed to list models of tenanetId {}. "
                        "{}".format(self._tenanetId, e))
            return []
    ...
    def _load_model_from_cloud(self, model_name, target_path):
        try:
            ...
            p = get_persistor(self.remote_storage)
            if p is not None:
                p.retrieve(model_name, self._tenanetId,self._botId, target_path)
            else:
