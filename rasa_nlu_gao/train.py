...
def do_train_in_worker(cfg,  # type: RasaNLUModelConfig
    ...
    try:
        _, _, persisted_path = do_train(cfg, data, path, tenanetId,
                                        botId, storage,
                                        component_builder)
        return persisted_path
    except BaseException as e:
        logger.exception("Failed to train tenanetId '{}'.".format(tenanetId))
        raise TrainingException(tenanetId, e)


def do_train(cfg,  # type: RasaNLUModelConfig
             data,  # type: Text
             path=None,  # type: Optional[Text]
             tenanetId=None,  # type: Optional[Text]
