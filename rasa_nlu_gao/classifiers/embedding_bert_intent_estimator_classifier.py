...
class EmbeddingBertIntentEstimatorClassifier(Component):
    ...
    defaults = {
        ...
        "config_proto": {
            ...
            "per_process_gpu_memory_fraction": 0.25 # this means use 50% of your gpu memory in max
        }
    }
    ...
    def train(self, training_data, cfg=None, **kwargs):
        ...
        train_x = X[indices[0:train_size]]
        train_y = intents_for_X[indices[0:train_size]]


        test_x = X[indices[train_size:data_size]]
        test_y = intents_for_X[indices[train_size:data_size]]
        ...
        feature_names = ['a_in']
        self.feature_columns = [tf.feature_column.numeric_column(key=k,shape=[1, train_x.shape[1]]) for k in feature_names]

        x_tensor_train = {'a_in': train_x}
        x_tensor_test = {'a_in': test_x}
        ...
        self.estimator.train(input_fn=lambda: self.input_fn(x_tensor_train,
                                                  ...
                                                  max_steps=800)
        # evaluate model
        results = self.estimator.evaluate(input_fn=lambda: self.input_fn(x_tensor_test,
                                                  test_y,
                                                  self.batch_size,
                                                  shuffle_num=1000,
