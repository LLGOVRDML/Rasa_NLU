...
class SmallCNN(object):
  ...
  def build_model(self, X):
    ...
    model = Sequential()
    model.add(Dense(750, activation='relu', input_dim=input_shape[0]))
  ...
  def encode_y(self, y):
    ...
    y_mapped = map(mapper, list(y))
    y_mapped = (list(y_mapped))
    transformed_y = np.array(y_mapped)
  ...
  def restore(self,path):
    new_model = K.models.load_model(path)
