...
from rasa_nlu_gao.model import DeleteFailError
...
class RasaNLU(object):
    ...
    def parse(self, request):
        ...
        tenanetId = parameter_or_default(request, "TenanetID", default="default")
        ...
        if 'q' not in request_params:
            ...
            dumped = json_to_string(
                {"error": {"statusCode": 404, "message": "Invalid parse parameter specified"}})
            returnValue(dumped)
        else:
    ...
    def extract_json(self, content):
        ...
        training_data = simplejson.loads(content).get("TrainingData")

        model_config = {
            "language": self.default_model_config['language'],
            "pipeline": self.default_model_config['pipeline'],
        }
        model_config["TenanetID"]=simplejson.loads(content).get("TenanetID")
        model_config["BotRecordId"] = simplejson.loads(content).get("BotRecordId")
        rasa_nlu_data = {}
        data = {}
        common_examples = []
        ...
        if training_data:
            ...
            data["rasa_nlu_data"]=rasa_nlu_data
    ...
    def train(self, request):
        ...
        try:
            ...
            response = yield self.data_router.start_train_process(
                    data_file, tenanetId,
                    RasaNLUModelConfig(model_config), botId)

            returnValue(json_to_string({'Code': '1',
                                        "Message": "",
                                        "TenanetID": tenanetId,
                                        "BotRecordId": botId}))
    ...
    def delete(self, request):
        ...
        try:
            ...
            response = yield self.data_router.delete(path="projects",tenanetId=tenanetId,botId=botId)
            returnValue(json_to_string(response))
